...
\begin{enumerate}
    ...
    \item[\textbf{2.2.8}] \textit{Demostrar que si $a$ y $b$ son elementos de un anillo $A$ para los que $(a, b) = A$, entonces $(a^n, b^m) = A$ para todo $n, m \in \mathbb{N}$.}

    \vspace{7px}

    \textsc{Demostración}: Probaremos que, bajo las hipótesis marcadas, se cumple que $1 \in (a^n, b)$ para todo $n \in \mathbb{N}$, relegando al lector la generalización ulterior.

    Por hipótesis, como $(a, b) = A$, en particular existen $x, y \in A$ tales que $ax + by = 1$. Elevando ambos lados de la igualdad a $n$ obtenemos la expresión \[(ax + by)^n = 1,\] y expandiendo el binomio y agrupando ingeniosamente  llegamos a que 

    \begin{equation*}
        \begin{split}
            (ax + by)^n &= \sum_{j=0}^{n} \binom{n}{j}(ax)^{n-j}(by)^j \\
            &= a^nx^n + \sum_{j=1}^{n} \binom{n}{j}(ax)^{n-j}(by)^j \\
            &= a^nx^n + b\sum_{j=1}^{n} \binom{n}{j}(ax)^{n-j}b^{j-1}y^j,
        \end{split}
    \end{equation*}

    donde, tras una rudimentaria comprobación, notamos se cumple $\sum_{j=1}^{n} \binom{n}{j} (ax)^{n-j}b^{j-1}y^j \in A$. Por subsecuente, concluimos que $1 \in (a^n, b)$, dejando unas condiciones más que favorables para que Ud. (\emph{función conativa}) dé una clausura apropiada a la demostración. \hfill{\textsc{Q.e.d.}}
    ...
    \textit{Demostrar que si $\mu(a)$ es irreducible, entonces $a$ es irreducible en $D$.}

    \vspace{7px}

    \textsc{Demostración}: Sea $a \in D$ tal que $\mu(a)$ es irreducible y supongamos que $a = bc, b, c \in D$. Por la primera propiedad de $\mu$ sabemos que \[\mu(a) = \mu(bc) = \mu(b)\mu(c),\] y como $\mu(a)$ es irreducible por hipótesis $\mu(b) \in \mathbb{Z}^* \lor \mu(c) \in \mathbb{Z}^*$. No obstante, como la imagen por $\mu$ es no negativa, podemos afirmar que $\mu(b) = 1 \lor \mu(c) = 1$, con lo que deducimos que $b \in D^* \lor c \in D^*$. Este razonamiento nos permite concluir que $a$ es irreducible. \hfill{\textsc{Q.e.d.}}

    \textit{Demostrar que $D$ es un dominio de factorización.}

    \vspace{7px}

    \textsc{Demostración}: Abordamos la demostración por inducción completa sobre el valor de $\mu(a)$. Para el caso base, si $\mu(a) = 1$, entonces $a$ es unidad y se puede factorizar.
    
    En cuanto al paso inductivo, dado $n \geq 1$, supongamos que $x \in D$ se puede factorizar si $1 \leq \mu(x) \leq n$ y veamos que si $\mu(a) = n + 1$ entonces $a$ también es factorizable. En efecto, sea $a \in D$ tal que $\mu(a) = n + 1$. Si $a$ fuese irreducible, ya habríamos términado, por lo que asumiremos que $a$ es reducible. Consecuentemente, existen $b, c \in D \setminus D^*$ cumpliendo $a = bc$, y al invocar la primera propiedad de $\mu$ nos queda que \[\mu(b)\mu(c) = \mu(a) = n + 1,\] de donde deducimos\footnote{Como $a \neq 0$ se tiene que $b, c \neq 0$ y, por otra parte, como $b$ y $c$ no son unidades, por la tercera propiedad, sus imagenes son distintas de $1$.} que $\mu(b), \mu(c) > 1$ y, por tanto, $1 < \mu(b), \mu(c) < n + 1$. En particular, tanto $b$ como $c$ satisfacen la hipótesis de inducción, por lo que son factorizables y así será $a$, pues si $b = up_1 \cdots p_k$ y $c = vq_1 \cdots q_l$ escribimos \[a = bc = (uv)p_1 \cdots p_kq_1 \cdots q_l.\] \hfill{\textsc{Q.e.d.}}

    \vspace{12px}

    \item[\textbf{2.5.4}] (Del documento auxiliar.) \textit{Sea $D$ un dominio euclídeo con función euclidea $\delta$. Probar que si $a, b \in D \setminus \{0\}$ son tales que $\delta(ab) = \delta(a)$, entonces $b$ es unidad.}

    \vspace{7px}

    \textsc{Demostración}: Definamos $p = ab$ y usemos el Lema 2.28 para deducir que \[\delta(p) = \delta(a) \leq \delta(x), \forall x \in (a) \setminus \{0\} \Longrightarrow (p) = (a),\] por lo que $p$ y $a$ son asociados o, dicho de forma más conveniente, $p = au$ con $u \in D^*$. Rememorando la definición de $p$ inferimos que $ab = au$, para concluir $b = u \in D^*$. \hfill{\textsc{Q.e.d.}}
\end{enumerate}
