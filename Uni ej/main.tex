...
\begin{itemize}
    ...
    $b_1 \leq b_2 \leq \cdots \leq b_n$, entonces $b_1 < 1 < b_n$.
    ...
    \[b_1b_2 \cdots b_n = (b_1b_2 \cdots b_{n-1})b_n \geq 1 \cdot b_n > 1,\]
    ...
    \begin{equation}
        b_1b_2 \cdots b_n = 1 \leq \frac{b_1 + b_2 + \cdots + b_n}{n},
        \label{eq:agm}
    \end{equation}
    ...
    \begin{itemize}
        ...
        A continuación, considerando $n + 1$ reales positivos ordenados $b_1b_2 \cdots b_{n+1}$ (no todos iguales) cuyo producto es $1$, veamos que se satisface $n + 1 < b_1 + b_2 + \cdots + b_{n + 1}$. Para ello, construimos la secuencia de números auxiliares $a_1, a_2, ..., a_n$ tales que
        ...
        agrupamiento todos los $a_i$ valgan $1$. En dicho escenario, se tiene trivialmente que $n = a_1 + a_2 + \cdots + a_n$, mientras que si no se da esta condición aplicamos 
        la hipótesis de inducción para alcanzar \eqref{eq:agmstrc}. En definitiva, no importa el escenario, sabemos que

        \[n \leq a_1 + a_2 + \cdots + a_n = b_1b_{n+1} + b_2 + \cdots + b_n.\]
    ...
    \[\frac{\sqrt[n]{a_1a_2\cdots a_n}}{x} = \sqrt[n]{b_1b_2\cdots b_n} = 1 \leq \frac{b_1 + b_2 + \cdots + b_n}{n},\]

    y, pasando la $x$ multiplicando, concluimos finalmente que

    \[\sqrt[n]{a_1a_2\cdots a_n} \leq \frac{x(b_1 + b_2 + \cdots + b_n)}{n} = \frac{a_1 + a_2 + \cdots + a_n}{n}.\]
...
\noindent \textsc{Demostración}: Supongamos, por reducción al absurdo, que $A$ es finito. Consecuentemente, existe $m = \max(A)$ y $M = \max(f(A))$. Definiendo $I_M = \{0, 1, 2, ..., M\}$ notamos que $f(I_M) \subseteq I_M$. En efecto, dado $0 \leq n \leq M$ se cumple, o bien $n \in A$, en cuyo escenario $0 \leq f(n) \leq M$ por definición de $M$, o bien $n \notin A$, donde se tiene trivialmente que $0 \leq f(n) < n \leq M$.

En consecuencia, la restricción $f\vert_{I_M} : I_M \longrightarrow I_M$ está bien definida y hereda la propiedad de inyectividad. Adicionalmente, como $I_M$ es un conjunto finito, se deduce que $f\vert_{I_M}$ es de igual manera sobreyectiva. Sin embargo, al contemplar la imagen de $M + 1$ sobre $f$ notamos que \[f(M + 1) \leq M,\] pues $M \geq m$ y, particularmente, $M + 1 \notin A$. Este hecho resulta en que $f(M + 1) \in I_M$, pero habíamos argumentado que todos los elementos de $I_M$ ya recibían del propio $I_M$, contradiciendo la cualidad de inyectividad asumida sobre $f$. En conclusión, $A$ ha de ser infinito. \hfill{\textsc{Q.e.d.}}
...
\noindent \textsc{Demostración}: En primer lugar, sin mucha agonía cognitiva podemos sacar de la definición que $a_1 = a_0 = a$, por lo que nos centramos en lo que resta de la demostración en $n > 1$.
...
\noindent\textbf{Problema} \, \textit{Sea $x \in \mathbb{R}$ un número real arbitrario y definimos la sucesión recurrente $(x_n)_n \subset \mathbb{R}$ dada por}
...
\noindent\textbf{Problema} \, \textit{Demostrar que si $a, b \in \mathbb{Z}$ son dos enteros coprimos, entonces $a^2$ y $b^2$ también lo son.}

\vspace{7px}

\noindent\textsc{Demostración}: Por ser $a, b$ coprimos, existen $x, y \in \mathbb{Z}$ tales que
...
\noindent de donde, elevando al cuadrado a ambos lados de la ecuación, nos deja que

\begin{equation}
    a^2x^2 + 2abxy + b^2y^2 = (ax + by)^2 = 1.
    \label{eq:eq004}
\end{equation}

\noindent Por otra parte, si además múltiplicamos \eqref{eq:eq003} por $2abxy$ en ambas partes, obtenemos la valiosa identidad

\[2a^2x^2by + 2b^2y^2ax = 2abxy,\]

\noindent por lo que, al sustituir en \eqref{eq:eq004} desembocamos finalmente en 

\[a^2x^2(2by + 1) + b^2y^2(2ax + 1) = a^2x^2 + 2a^2x^2by + 2b^2y^2ax + b^2y^2 = a^2x^2 + 2abxy + b^2y^2 = 1,\]

\noindent concluyendo así que $a^2$ y $b^2$ son comprimos, pues hemos conseguido expresar $1$ como combinación lineal de $a^2$ y $b^2$. \hfill{\textsc{Q.e.d.}}

\vspace{15px}

\noindent \textbf{Problema} \, \textit{Sean $a, b$ son dos enteros positivos tales que $\mcd(a, b) = 1$ y $ab = c^n$. Probar que $a = x^n$ y $b = y^n$ para $x$ e $y$ apropiados.}

\vspace{5px}

\noindent \textsc{Demostración}: Nuestro foco va a estar en probar que $d = \mcd(a, c)$ y $g = \mcd(b, c)$ verifican que $a = d^n$ y $b = g^n$. Comenzamos expresando $c = dc'$ para sustituir \[ab = c^n = d^nc'^n\]

\vspace{12px}

\noindent \textbf{Problema} \, \textit{Si $m \neq n$, estudiar el valor de $\mcd(a^{2^m} + 1, a^{2^n} + 1)$ en función de $a > 1$.}

\vspace{5px}

\noindent \textsc{Solución}: Empezamos con paso firme definiendo $A_n = a^{2^n} + 1$. A continuación, vamos a deducir que \[A_n \mid A_m + A_n - 2,\] para todo $m > n$. En efecto, como $m > n$, podemos escribir $m = n + k$, donde $k$ es positivo, para desarrollar

\begin{equation*}
    \begin{split}
        A_m + A_n - 2 &= a^{2^m} + 1 + a^{2^n} + 1 - 2 \\
        &= a^{2^m} + a^{2^n} \\
        &= a^{2^n2^k} + a^{2^n} \\
        &= \left(a^{2^n}\right)^{2^k} + a^{2^n} \\
        &= a^{2^n}\left(\left(a^{2^n}\right)^{2^k-1} + 1\right).
    \end{split}
\end{equation*}

Adicionalmente, y como pieza clave, notamos que el exponente $2^k - 1$ es impar, pues $k > 0$, por lo que podemos aplicar la mundialmente conocida identidad \[x^\mu + 1 = (x + 1)(x^{\mu-1} - x^{\mu-2} + \cdots - x + 1),\] válida cuando $\mu$ es impar, tomando en nuestro caso de interés $\mu = 2^k-1$ y $x = a^{2^n}$. En consecuencia, inferimos que \[A_m + A_n - 2 = a^{2^n}\left(a^{2^n} + 1\right)S = a^{2^n}A_nS,\] donde $S$ es el correspondiente sumatorio $\sum_{j=0}^{\mu-1} (-1)^jx^j$. Dicho de otra manera, hemos probado que $A_n \mid A_m + A_n - 2$.

Este resultado, unido al hecho trivial de que $A_n \mid A_n$, nos permite afirmar que $A_n \mid A_m - 2$ para cualesquiera $m > n$. Por subsiguiente, denotando $d = \mcd(A_n, A_m)$, notamos que, por un lado, juntando $d \mid A_n$ y $A_n \mid A_m - 2$ llegamos a que $d \mid A_m - 2$ y, por otro lado, como $d \mid A_m$ por definición, concluimos que \[d \mid 2 = A_m - (A_m - 2).\]

Argumentando como previamente nos deja dos posibilidades para $d$, siendo éstas $d = 1$ o $d = 2$. Mediante una irrisoria y poco estimulante distinción de casos, concluimos que $d = 1$ si $a$ es par, mientras que $d = 2$ en caso contrario. $\hfill\square$

\bigskip

\noindent\textbf{Proposición} \, \textit{Dados $u, v \in \mathbb{R}$ se cumplen las siguientes propiedades:}

\begin{itemize}
        \item[\textit{(i)}] \textit{Si $\cos u = \cos v$ y $\sin u = \sin v$, entonces $u - v \in 2\pi\mathbb{Z}$.}

        \textsc{Demostración}: Supuesto $\cos u = \cos v$ y $\sin u = \sin v$, desarrollamos la expresión $\cos(u - v)$:
        \begin{equation*}
            \begin{split}
                \cos(u-v) &= \cos u \cos v + \sin u \sin v \\
                &= \cos^2u + \sin u \sin v \quad [\cos u = \cos v] \\
                &= \cos^2u + \sin^2u \quad [\sin u = \sin v] \\
                &= 1.
            \end{split}
        \end{equation*}
        Por la periocidad de la función $\cos$ sabemos que esto implica que $u - v = 2\pi k$, donde $k \in \mathbb{Z}$. Esto es, se cumple que $u - v \in 2\pi\mathbb{Z}$. \hfill{\textsc{Q.e.d.}}

        \medskip
    
        \item[\textit{(ii)}] \textit{Si $\sin u = \sin v$, entonces $u - v \in 2\pi\mathbb{Z}$ o $u + v \in \pi + 2\pi\mathbb{Z}$.}

            \textsc{Demostración}: En primer lugar, notamos que se cumple \[\cos^2u = 1 - \sin^2u = 1 - \sin^2v = \cos^2v,\] por lo que deducimos que $\cos u = \pm \cos v$. En el caso de $\cos u = \cos v$, se dan las hipótesis de (i), por lo que inferimos que $u - v \in 2\pi\mathbb{Z}$. En caso contrario, si $\cos u = -\cos v$, desplegamos la expresión $\cos(u+v)$ para notar que
        \begin{equation*}
            \begin{split}
                \cos(u+v) &= \cos u \cos v - \sin u \sin v \\
                &= -\cos^2u - \sin u \sin v \quad \, [\cos u = -\cos v] \\
                &= -\cos^2u - \sin^2u \qquad \quad [\sin u = \sin v] \\
                &= -1.
            \end{split}
        \end{equation*}
        Para terminar, volvemos a actuar bajo tutela de la periocidad de la función $\cos$ para concluir que $u + v = \pi + 2\pi k$, con $k \in \mathbb{Z}$, o lo que es lo mismo, $u + v \in \pi + 2\pi\mathbb{Z}$. \hfill{\textsc{Q.e.d.}} 

        \medskip

        \item[\textit{(iii)}] \textit{Si $\cos u = \cos v$, entonces $u - v \in 2\pi\mathbb{Z}$ o $u + v \in 2\pi\mathbb{Z}$.}

        \textsc{Demostración}: Manipulación y argumentación cortadas con el mismo filo que (ii). \hfill{\textsc{Q.e.d.}}
\end{itemize}
