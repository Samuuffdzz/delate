...
\noindent \textsc{Demostración}: Supongamos, por reducción al absurdo, que $A$ es finito. Consecuentemente, existe $m = \max(A)$ y $M = \max(f(A))$. Definiendo $I_M = \{0, 1, 2, ..., l\}$ notamos que $f(I_M) \subseteq I_M$. En efecto, dado $0 \leq n \leq M$ se cumple, o bien $n \in A$, en cuyo escenario $f(n) \leq M$ por definición de $M$, o bien $n \notin A$, donde se tiene trivialmente que $f(n) < n \leq M$. Por otra parte, la cota $f(n) \geq 0$ se tiene gratuitamente, pues la imagen de $f$ es $\mathbb{N} = \{0, 1, 2, 3, ...\}$ en nuestro contexto.
